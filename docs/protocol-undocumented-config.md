...
| 0x05 | Plus firmware file transport | `[02][05][outer][outer_done][file_done][inner u16][size u16][0x02][0x00 x6][data@0x10]`; 4096-byte outer blocks, chunks <=1008 | - | static Plus `20GBD9901` updater trace only; never sent by this library |
...
4096-byte/command-0x05 uploader. This is a static trace, not a captured or
physically observed firmware update; payload format, device-side authorization,
bootloader behavior and recovery remain unknown. See
`firmware-update-research.md` for the byte map, reproducible offsets and the
read-only capture parser. Output command 0x02 and its `[0x03][0x07]` finalizer
remain a separate unclassified upload form. The 0x01 upload has four app call
sites, but the meaning and valid range of its `param` byte remain unknown.
